...
export default function Blog() {
  return (
    <div>
      <h1 className='text-4xl font-bold text-center text-purple-400'>Cybersecurity Blog</h1>
      <div className='grid md:grid-cols-3 gap-6 mt-6'>
        {posts.map(post => (
          <div key={post.id} className='flex flex-col justify-stretch bg-gray-800 rounded-lg shadow-lg overflow-hidden p-4'>
            ...
            <Link href={'/blog/' + post.id} className='w-[115px] text-center mt-4 px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition'>
              Read More
            </Link>
          </div>
        ))}
